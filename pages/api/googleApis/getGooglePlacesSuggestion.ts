import {
  ...
  const getGooglePlacesSuggestion = async (request, response,) => {
    ...
    if (request.query.isAutoComplete === 'false') {
        ...
        request.query.zipCode ? (input = request.query.zipCode) : (input = request.query.name)
    }
    ...
    console.log('what is requested?', requested,
    )
...
export default getGooglePlacesSuggestion